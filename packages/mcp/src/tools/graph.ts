...
import { readAllPlans, findPlan, collectRefs, resolveRef } from '@pc-ctx/core';
import { toJson, toError } from '../format.js';

export function registerGraphTool(server: McpServer, ctx: { plansDir: string; roadmapsDir: string; researchDir: string }) {
  server.tool(
    'graph',
    ...
    async ({ slug }) => {
      try {
        ...
        if (slug) {
          ...
          if (!plan) return { content: [{ type: 'text' as const, text: toJson({ error: `Plan "${slug}" not found.` }) }], isError: true as const };
          ...
          for (const doc of allDocs) {
            ...
            if (collectRefs(doc).some(r => r === `plan:${slug}`)) {
              edges.push({ from: doc.slug, to: slug, type: 'backlink', description: `referenced by ${doc.frontmatter.title}` });
            }
          }
        } else {
          for (const doc of allDocs) {
            for (const r of collectRefs(doc)) {
              const resolved = resolveRef(r, ctx.plansDir, ctx.roadmapsDir, ctx.researchDir);
              edges.push({ from: doc.slug, to: resolved.label, type: resolved.type, description: resolved.description });
            }
          }
        }
