...
import { findPlan, collectRefs, resolveRef, readAllPlans } from '@pc-ctx/core';
import { toJson, toError, notFound } from '../format.js';

export function registerReferencesTool(server: McpServer, ctx: { plansDir: string; roadmapsDir: string; researchDir: string }) {
  server.tool(
    'plan_references',
